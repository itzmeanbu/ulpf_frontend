  ...
  const upload = async () => {
    ...
    try {
      ...
      for (const line of asJson) {
        try { results.push(await logService.uploadLog(line, sourceType)); ok++; }
    ...
    <PageContainer>
      <div className="pg-stack">
        ...
        {uploadResults.length > 0 && (
          <div className="panel">
            <div className="panel-header"><h2>Just processed</h2></div>
            <div className="panel-body pg-stack">
              {uploadResults.map((r) => (
                <div key={r.eventId} className="pg-msg ok" style={{ fontSize: '0.95rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10 }}>
                  <span>{r.readableSummary}</span>
                  <button className="pg-btn small ghost" onClick={() => open(r)}>Full details</button>
                </div>
              ))}
            </div>
          </div>
