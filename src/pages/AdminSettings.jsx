...
function Options({ title, intro, fields, load, save }) {
  const [cfg, setCfg] = useState(null);
  const [msg, setMsg] = useState(null);
  useEffect(() => {
    load().then(setCfg).catch((e) => setMsg({ type: 'err', text: e.message }));
  }, [load]);
  ...
  return (
    <div className="pg-stack">
      ...
      <Options
        ...
        fields={[
          { key: 'anomalyDetection', type: 'bool', label: 'Watch failed logins', help: 'Creates an alert when someone fails to log in many times within 5 minutes.' },
          { key: 'threatCorrelation', type: 'bool', label: 'Watch repeated IPs', help: 'Creates an alert when the same IP shows up in many logs within 10 minutes.' },
          { key: 'automaticAlerts', type: 'bool', label: 'Create alerts automatically', help: 'If off, the site will not create alerts for repeated IPs.' },
          { key: 'aiMasking', type: 'bool', label: 'AI masking (not working yet)', help: 'Does nothing for now. Hiding data uses fixed rules, not AI.' },
          { key: 'sensitivity', type: 'select', label: 'Sensitivity', options: ['low', 'medium', 'high'], help: 'High = alerts appear after fewer events (more alerts). Low = needs more events (fewer alerts).' },
        ]}
      />
      <Options
        ...
        fields={[
          { key: 'sessionTimeoutMinutes', type: 'num', label: 'Login lasts (minutes)', help: 'After this time the user must sign in again. Applies to new logins.' },
